...
function update_parameters(){
    ...
    if (Q == 'customOption') {Q = parseFloat(document.getElementById("vent-custom").value);
    }else {Q = parseFloat(Q);}
    v = document.getElementById("airflow").value;
    if (v == 'customOption') {v = parseFloat(document.getElementById("airflow-custom").value);
    }else {v = parseFloat(v);}
    ...
    if (frac_speak == 'customOption') {frac_speak = parseFloat(document.getElementById("talk-custom").value) / 100;
    }else {frac_speak = parseFloat(frac_speak) / 100;}
    mask_ex = document.getElementById("inf-mask").value;
    if (mask_ex == 'customOption') {mask_ex = parseFloat(document.getElementById("inf-mask-custom").value) / 100;
    }else {mask_ex = parseFloat(mask_ex);}

    // susceptible person parameters: p, mask_in
    mask_in = document.getElementById("occ-mask").value;
    if (mask_in == 'customOption') {mask_in = parseFloat(document.getElementById("occ-mask-custom").value) / 100;
    }else {mask_in = parseFloat(mask_in);}
    p = document.getElementById("occ-act").value;
    if (p == 'customOption') {p = parseFloat(document.getElementById("occ-act-custom").value);
    }else {p = parseFloat(p);}
    ...
    if (R == 'customOption') {R = parseFloat(document.getElementById("viral-load-custom").value) / Math.log(10);
    }else {R = parseFloat(R);}
...
function Run_moded(){
    ...
    for (let i=0; i<tt.length; i++){S[i] = (1-mask_ex)*(q*(1-frac_speak)+10*q*frac_speak) * delta_t;}
    var n_temp1 = parseInt(break_start*3600/delta_t); var n_temp2 = n_temp1 + parseInt(time_break * 3600 /delta_t);
    for (let i=n_temp1; i<n_temp2;i++){S[i]=0;}
    ...
    for (let i=0;i<tt.length;i++){
        ...
        for (let j=0;j<yy.length;j++){
            I_short[i][j] = Math.exp((-1)*(yy[j]-y_o)**2 / (4*K*tt[i])) + Math.exp((-1)*(yy[j]+y_o)**2 / (4*K*tt[i]));
            for (let m=1;m<4;m++){
                I_short[i][j] = I_short[i][j] + Math.exp((-1)*(yy[j]-y_o-2*m*w)**2 / (4*K*tt[i])) + Math.exp((-1)*(yy[j]+y_o-2*m*w)**2 / (4*K*tt[i]));
                I_short[i][j] = I_short[i][j] + Math.exp((-1)*(yy[j]-y_o+2*m*w)**2 / (4*K*tt[i])) + Math.exp((-1)*(yy[j]+y_o+2*m*w)**2 / (4*K*tt[i]));
            }
            I_short[i][j] = 1/(4*Math.PI*K*tt[i]) * Math.exp( (-1)*tt[i]*(Q+s+d)) * I_short[i][j];
        }
    }
    ...
    for(let j=0;j<yy.length;j++){
        for(let k=0;k<xx.length;k++){    
            C_room[0][j][k] = S[0] * I_short[0][j] * ( Math.exp((-1)*(xx[k]-x_o-v*tt[0])**2 / (4*K*tt[0])) + Math.exp((-1)*(xx[k]+x_o+v*tt[0])**2 / (4*K*tt[0])) ) / (h/2);
            Part_in[0][j][k] = 0.5 * C_room[0][j][k] * delta_t;
            P_room[0][j][k] = 1 - Math.exp((-1)*p*(1-mask_in)*(1+r)*I_o*Part_in[0][j][k]);

            for(let i=1;i<tt.length;i++){
                if(i<n_temp1){
                    C_room[i][j][k] = C_room[i-1][j][k] + S[i] * I_short[i][j] * ( Math.exp((-1)*(xx[k]-x_o-v*tt[i])**2 / (4*K*tt[i])) + Math.exp((-1)*(xx[k]+x_o+v*tt[i])**2 / (4*K*tt[i])) ) / (h/2);
                } else if(i>=n_temp2){
                    C_room[i][j][k] = C_room[i-1][j][k] + S[0] * I_short[i][j] * ( Math.exp((-1)*(xx[k]-x_o-v*tt[i])**2 / (4*K*tt[i])) + Math.exp((-1)*(xx[k]+x_o+v*tt[i])**2 / (4*K*tt[i])) ) / (h/2);
                    C_room[i][j][k] = C_room[i][j][k] + S[i] * I_short[i-n_temp2][j] * ( Math.exp((-1)*(xx[k]-x_o-v*tt[i])**2 / (4*K*tt[i])) + Math.exp((-1)*(xx[k]+x_o+v*tt[i])**2 / (4*K*tt[i])) ) / (h/2);
                    C_room[i][j][k] = C_room[i][j][k] - S[0] * I_short[i-n_temp1][j] * ( Math.exp((-1)*(xx[k]-x_o-v*tt[i-n_temp1])**2 / (4*K*tt[i-n_temp1])) + Math.exp((-1)*(xx[k]+x_o+v*tt[i-n_temp1])**2 / (4*K*tt[i-n_temp1])) ) / (h/2);
                } else{
                    C_room[i][j][k] = C_room[i-1][j][k] + S[0] * I_short[i][j] * ( Math.exp((-1)*(xx[k]-x_o-v*tt[i])**2 / (4*K*tt[i])) + Math.exp((-1)*(xx[k]+x_o+v*tt[i])**2 / (4*K*tt[i])) ) / (h/2);
                    C_room[i][j][k] = C_room[i][j][k] - S[0] * I_short[i-n_temp1][j] * ( Math.exp((-1)*(xx[k]-x_o-v*tt[i-n_temp1])**2 / (4*K*tt[i-n_temp1])) + Math.exp((-1)*(xx[k]+x_o+v*tt[i-n_temp1])**2 / (4*K*tt[i-n_temp1])) ) / (h/2);
                }
            ...
            for(let i=1;i<tt.length;i++){
                if(S[i]>0){Part_in[i][j][k] = Part_in[i-1][j][k] + 0.5*(C_room[i-1][j][k] + C_room[i][j][k])*delta_t;}
                else{Part_in[i][j][k] = Part_in[i-1][j][k];}
                P_room[i][j][k] = 1 - Math.exp((-1)*p*(1-mask_in)*(1+r)*I_o*Part_in[i][j][k]);
            }
        }
    }
    ...
    risk_avg_print.innerHTML = "Average infection risk from airborne transmission in the room is " + round(P_output, 1) + "%.";
    inf_print.innerHTML = "ie. <output>" + round(P_output, 0) + "</output> out of <input type='number' value='100' min='0' step='1' style='width:45px' oninput='this.previousElementSibling.value = round(this.value/100*" + P_output +",0)'> </input> people in the room will likely be infected.";
    ...
    function add_modes(){
        if(modes > m_x){
            ...
            for(let j=0;j<yy.length;j++){
                for(let k=0;k<xx.length;k++){
                    temp[0][j][k] = S[0] * I_short[0][j] / (h/2) * (Math.exp((-1)*(xx[k]-x_o-v*tt[0]+2*modes*l)**2 / (4*K*tt[0])) + Math.exp((-1)*(xx[k]+x_o+v*tt[0]+2*modes*l)**2 / (4*K*tt[0])) );    
                    temp[0][j][k] = temp[0][j][k] + S[0] * I_short[0][j] / (h/2) * (Math.exp((-1)*(xx[k]-x_o-v*tt[0]-2*modes*l)**2 / (4*K*tt[0])) + Math.exp((-1)*(xx[k]+x_o+v*tt[0]-2*modes*l)**2 / (4*K*tt[0])) );    
                    for(let i=1;i<tt.length;i++){
                        if(i<n_temp1){
                            temp[i][j][k] = temp[i-1][j][k] + S[i] * I_short[i][j] / (h/2) * (Math.exp((-1)*(xx[k]-x_o-v*tt[i]+2*modes*l)**2 / (4*K*tt[i])) + Math.exp((-1)*(xx[k]+x_o+v*tt[i]+2*modes*l)**2 / (4*K*tt[i])) ); 
                            temp[i][j][k] = temp[i][j][k] + S[i] * I_short[i][j] / (h/2) * (Math.exp((-1)*(xx[k]-x_o-v*tt[i]-2*modes*l)**2 / (4*K*tt[i])) + Math.exp((-1)*(xx[k]+x_o+v*tt[i]-2*modes*l)**2 / (4*K*tt[i])) );    
                        } else if(i>=n_temp2){
                            temp[i][j][k] = temp[i-1][j][k] + S[0] * I_short[i][j] / (h/2) * (Math.exp((-1)*(xx[k]-x_o-v*tt[i]+2*modes*l)**2 / (4*K*tt[i])) + Math.exp((-1)*(xx[k]+x_o+v*tt[i]+2*modes*l)**2 / (4*K*tt[i])) ); 
                            temp[i][j][k] = temp[i][j][k] + S[0] * I_short[i][j] / (h/2) * (Math.exp((-1)*(xx[k]-x_o-v*tt[i]-2*modes*l)**2 / (4*K*tt[i])) + Math.exp((-1)*(xx[k]+x_o+v*tt[i]-2*modes*l)**2 / (4*K*tt[i])) ); 
                            temp[i][j][k] = temp[i][j][k] - S[0] * I_short[i-n_temp1][j] / (h/2) * ( Math.exp((-1)*(xx[k]-x_o-v*tt[i-n_temp1]+2*modes*l)**2 / (4*K*tt[i-n_temp1])) + Math.exp((-1)*(xx[k]+x_o+v*tt[i-n_temp1]+2*modes*l)**2 / (4*K*tt[i-n_temp1])) );   
                            temp[i][j][k] = temp[i][j][k] - S[0] * I_short[i-n_temp1][j] / (h/2) * ( Math.exp((-1)*(xx[k]-x_o-v*tt[i-n_temp1]-2*modes*l)**2 / (4*K*tt[i-n_temp1])) + Math.exp((-1)*(xx[k]+x_o+v*tt[i-n_temp1]-2*modes*l)**2 / (4*K*tt[i-n_temp1])) );   
                            temp[i][j][k] = temp[i][j][k] + S[i] * I_short[i-n_temp2][j] / (h/2) * ( Math.exp((-1)*(xx[k]-x_o-v*tt[i]+2*modes*l)**2 / (4*K*tt[i-n_temp2])) + Math.exp((-1)*(xx[k]+x_o+v*tt[i]+2*modes*l)**2 / (4*K*tt[i-n_temp2])) );
                            temp[i][j][k] = temp[i][j][k] + S[i] * I_short[i-n_temp2][j] / (h/2) * ( Math.exp((-1)*(xx[k]-x_o-v*tt[i]-2*modes*l)**2 / (4*K*tt[i-n_temp2])) + Math.exp((-1)*(xx[k]+x_o+v*tt[i]-2*modes*l)**2 / (4*K*tt[i-n_temp2])) );
                        } else{
                            temp[i][j][k] = temp[i-1][j][k] + S[0] * I_short[i][j] / (h/2) * (Math.exp((-1)*(xx[k]-x_o-v*tt[i]+2*modes*l)**2 / (4*K*tt[i])) + Math.exp((-1)*(xx[k]+x_o+v*tt[i]+2*modes*l)**2 / (4*K*tt[i])) ); 
                            temp[i][j][k] = temp[i][j][k] + S[0] * I_short[i][j] / (h/2) * (Math.exp((-1)*(xx[k]-x_o-v*tt[i]-2*modes*l)**2 / (4*K*tt[i])) + Math.exp((-1)*(xx[k]+x_o+v*tt[i]-2*modes*l)**2 / (4*K*tt[i])) ); 
                    ...
                    for(let i=1;i<tt.length;i++){
                        ...
                        if(S[i]>0){
                            Part_in[i][j][k] = Part_in[i-1][j][k] + 0.5*(C_room[i][j][k]+C_room[i-1][j][k])*delta_t;
                        }else{Part_in[i][j][k] = Part_in[i-1][j][k];}
                    }
                    for(let i=0;i<tt.length;i++){
                        P_room[i][j][k] = 1 - Math.exp((-1)*p*(1-mask_in)*(1+r)*I_o*Part_in[i][j][k]);
            ...
            var chart_title = 'Risk at ' + round(tt[n_plot*(modes-1)]/60, 0) + ' minutes';
            chart_title_display.innerHTML=chart_title;
            var dataptP_room = [{
                x: xx,
                y: yy,
            ...
            Plotly.newPlot(P_contour_chart,dataptP_room,{margin:{t:0}});
            var P_output = avg_free(P_room[n_plot*(modes-1)],l,w,delta_x,delta_y) * 100;
            risk_avg_print.innerHTML = "Average infection risk from airborne transmission in the room is " + round(P_output, 1) + "%.";
            inf_print.innerHTML = "ie. <output>" + round(P_output, 0) + "</output> out of <input type='number' value='100' min='0' step='1' style='width:45px' oninput='this.previousElementSibling.value = round(this.value/100*" + P_output +",0)'> </input> people in the room will likely be infected.";
...
function Run_no_modes(){
    ...
    function add_point(){    
        if(n_points >= n_total){
            ...
            document.getElementById('chart_time').innerHTML = "Infection risk at the end of the event";
            ...
            risk_avg_print.innerHTML = "Average infection risk from airborne transmission in the room is " + round(P_avg*100, 1) + "%.";
            inf_print.innerHTML = "ie. <output>" + round(P_avg*100, 0) + "</output> out of <input type='number' value='100' min='0' step='1' style='width:45px' oninput='this.previousElementSibling.value = round(this.value*" + P_avg +",0)'> </input> people in the room will likely be infected.";
